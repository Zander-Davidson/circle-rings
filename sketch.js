...
function setup()
{
    ...
    // Color Alpha Slider
    color_alpha_slider_label = createP(LABELS.ALPHA);
    color_alpha_slider_label.parent(colorControlsWrapper);
    color_alpha_slider_label.class('alpha-slider-label');
    ...
    // Reset button
    reset_button = createButton("randomize all");
    reset_button.parent(controlsContainer);
    reset_button.mousePressed(() => {
        // Reset sliders to default values
        num_rings_slider.value(randomInRange(NUM_RINGS_CONFIG.min, 16, NUM_RINGS_CONFIG.step));
        inner_ring_radius_slider.value(randomInRange(INNER_RING_RADIUS_CONFIG.min, INNER_RING_RADIUS_CONFIG.max, INNER_RING_RADIUS_CONFIG.step));
        outer_ring_radius_slider.value(randomInRange(OUTER_RING_RADIUS_CONFIG.min, OUTER_RING_RADIUS_CONFIG.max, OUTER_RING_RADIUS_CONFIG.step));
        num_circles_slider.value(randomInRange(NUM_CIRCLES_CONFIG.min, 8, NUM_CIRCLES_CONFIG.step));
        small_circle_radius_slider.value(randomInRange(SMALL_CIRCLE_RADIUS_CONFIG.min, SMALL_CIRCLE_RADIUS_CONFIG.max, SMALL_CIRCLE_RADIUS_CONFIG.step));
        large_circle_radius_slider.value(randomInRange(LARGE_CIRCLE_RADIUS_CONFIG.min, LARGE_CIRCLE_RADIUS_CONFIG.max, LARGE_CIRCLE_RADIUS_CONFIG.step));
        spiral_drift_slider.value(0.0);
        spiral_drift_enabled_checkbox.checked(true);
        radial_blur_enabled_checkbox.checked(false);
        generate_random_colors(NUM_CIRCLES_CONFIG.max * NUM_RINGS_CONFIG.max * 3);
    });

    // created by label
    const createdByLabel = createA(LINK_TO_REPO, LABELS.CREATED_BY);
    createdByLabel.parent(controlsContainer);
    createdByLabel.class('created-by-label');
}
...
function draw() 
{
    ...
    inner_ring_radius_slider_label.html(LABELS.INNER_RING_RADIUS)
    ...
    small_circle_radius_slider_label.html(LABELS.SMALL_CIRCLE_RADIUS)
